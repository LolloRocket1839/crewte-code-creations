...
import { Trash2, Check, X, Paperclip, Pencil } from 'lucide-react';
...
export function ExpenseItem({ expense, index = 0 }: ExpenseItemProps) {
  ...
  const handleSwipeEdit = () => {
    // Trigger the edit dialog programmatically
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Actions row - hidden on mobile (use swipe), visible on desktop */}
        <div className="hidden md:flex items-center justify-end gap-1 mt-2 pl-[92px]">
          {/* Receipt Icon */}
          {expense.receipt_url && (
            <Button
              variant="ghost"
        ...
        {/* Mobile receipt button - stays visible since swipe only has edit/delete */}
        {expense.receipt_url && (
          <div className="flex md:hidden items-center justify-end mt-2 pl-[92px]">
            <Button
              variant="ghost"
              size="icon"
              aria-label="Visualizza ricevuta"
        ...
        {/* Hidden edit trigger for swipe action */}
        <EditExpenseDialog 
          expense={expense} 
